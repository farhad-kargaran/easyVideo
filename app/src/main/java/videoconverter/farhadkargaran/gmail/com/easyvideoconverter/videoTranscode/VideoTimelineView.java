...
public class VideoTimelineView extends View {
    ...
    private void init(Context context) {
        ...
        pickDrawable = Helper.getDrawable(R.drawable.videotrimmer);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        int width = getMeasuredWidth() - Helper.dp(32);
        int startX = (int)(width * progressLeft) + Helper.dp(16);
        int endX = (int)(width * progressRight) + Helper.dp(16);

        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            int additionWidth = Helper.dp(12);
            ...
            if (pressedLeft) {
                ...
                if (startX < Helper.dp(16)) {
                    startX = Helper.dp(16);
                } else if (startX > endX) {
                    startX = endX;
                }
                progressLeft = (float)(startX - Helper.dp(16)) / (float)width;
                if (delegate != null) {
                ...
                if (endX < startX) {
                    endX = startX;
                } else if (endX > width + Helper.dp(16)) {
                    endX = width + Helper.dp(16);
                }
                progressRight = (float)(endX - Helper.dp(16)) / (float)width;
                if (delegate != null) {
    ...
    private void reloadFrames(int frameNum) {
        ...
        if (frameNum == 0) {
            frameHeight = Helper.dp(40);
            framesToLoad = (getMeasuredWidth() - Helper.dp(16)) / frameHeight;
            frameWidth = (int) Math.ceil((float)(getMeasuredWidth() - Helper.dp(16)) / (float)framesToLoad);
    ...
    protected void onDraw(Canvas canvas) {
        int width = getMeasuredWidth() - Helper.dp(36);
        int startX = (int)(width * progressLeft) + Helper.dp(16);
        int endX = (int)(width * progressRight) + Helper.dp(16);

        canvas.save();
        canvas.clipRect(Helper.dp(16), 0, width + Helper.dp(20), Helper.dp(44));
        if (frames.isEmpty() && currentTask == null) {
            ...
            for (Bitmap bitmap : frames) {
                if (bitmap != null) {
                    canvas.drawBitmap(bitmap, Helper.dp(16) + offset * frameWidth, Helper.dp(2), null);
                }
                offset++;
            }
        }

        canvas.drawRect(Helper.dp(16), Helper.dp(2), startX, Helper.dp(42), paint2);
        canvas.drawRect(endX + Helper.dp(4), Helper.dp(2), Helper.dp(16) + width + Helper.dp(4), Helper.dp(42), paint2);

        canvas.drawRect(startX, 0, startX + Helper.dp(2), Helper.dp(44), paint);
        canvas.drawRect(endX + Helper.dp(2), 0, endX + Helper.dp(4), Helper.dp(44), paint);
        canvas.drawRect(startX + Helper.dp(2), 0, endX + Helper.dp(4), Helper.dp(2), paint);
        canvas.drawRect(startX + Helper.dp(2), Helper.dp(42), endX + Helper.dp(4), Helper.dp(44), paint);
        canvas.restore();
        ...
        pickDrawable.setBounds(endX - drawableWidth / 2 + Helper.dp(4), getMeasuredHeight() - drawableHeight, endX + drawableWidth / 2 + Helper.dp(4), getMeasuredHeight());
        pickDrawable.draw(canvas);
    }
}
